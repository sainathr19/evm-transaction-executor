...
export class SenderRegistry {
  ...
  add(chainId: number, sender: Address, pool: NoncePool): SenderState {
    const state = new SenderState(pool)
    this.#states.set(key(chainId, sender), state)
    return state
  }

  get(chainId: number, sender: Address): SenderState {
...
function key(chainId: number, sender: Address): string {
