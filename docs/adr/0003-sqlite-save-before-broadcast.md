...
- Two tables. The initial columns may be refined during implementation.
  - **`transactions`:** id, kind (`request` or `gap_fill`), idempotency key (unique) and request hash, chain id, sender, to, value, data, status, nonce, gas limit, current hash, receipt (JSON), error code and message, timestamps.
